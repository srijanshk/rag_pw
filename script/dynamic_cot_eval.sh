...
run_job () {
  ...
  python multistep_rag.py \
      ...
      --split        test \
      --seed         1234 \
      --out_path     results/${NAME}_MULTI_STAGE_COT_${STAMP}.jsonl \
      --batch        $BATCH \
      --max_new      $MAX_NEW \
      --wandb_project $WPROJ \
      --wandb_run_name ${NAME}__MULTI_STAGE_COT_llama3_${STAMP} \
